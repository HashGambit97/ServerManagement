...
function Get-DfsrBacklogStatus
{
    ...
    .PARAMETER ComputerName
        ...
        If you do not specify a computer name, the cmdlet uses the local computer.

    .PARAMETER FolderName
        Specifies an array of names of replicated folders. If you do not specify this parameter, the cmdlet returns backlog information for all replicated folders on the specified computer.
        ...
        foreach ($Computer in $ComputerName)
        {
            ...
            foreach ($Folder in $DfsrFolderInfo)
            {
                ...
                foreach ($Partner in $InboundPartner)
                {
                    try
                    {
                        $PartnerFolderInfo = Get-CimInstance -ComputerName $Partner.PartnerName -Namespace 'root\MicrosoftDFS' -ClassName 'DfsrReplicatedFolderInfo' -ErrorAction Stop
                        $PartnerFolder = $PartnerFolderInfo | Where-Object { $PSItem.ReplicatedFolderGuid -eq $Folder.ReplicatedFolderGuid }
                    ...
                    catch
                    {
                        ...
                        $PartnerFolderInfo = Get-WmiObject -ComputerName $Partner.PartnerName -Namespace 'root\MicrosoftDFS' -Class 'DfsrReplicatedFolderInfo'
                        $PartnerFolder = $PartnerFolderInfo | Where-Object { $PSItem.ReplicatedFolderGuid -eq $Folder.ReplicatedFolderGuid }
